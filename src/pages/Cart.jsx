...
import { CardsItem } from '../components';
...
function Cart() {
  ...
  const {totalPrice, totalCount, items}
   = useSelector(({cart}) => cart); 
  //как вытащить items? это ж объект 
  
  const addedGuides = Object.keys(items).map((key)=>{
    return items [key].items[0]; //для items мы прописали
    //ключи 
  });
  ...
  const onClearCart = () => {
    if (window.confirm
      ...
      dispatch(clearCart()); //метод который отвечает за очистку корзины, dispatch будет отправлять объект в редакс 
...
<path d="M11.6666 9.16667V14.1667" stroke="#B6B6B6" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/>
            ...
            <span onClick={onClearCart}>Очистить вишлист</span>
          </div>
        </div>
    ...
    name = {obj.name} 
    type = {obj.type} 
    size = {obj.size}//говорим что нужно пробежаться по каждому объекту и потом взять картАйтем оттуда взять name из объекта а потом взять тип и далее взять size 
  ...
  />
        ...
        <div className="cart__bottom">
          <div className="cart__bottom-details">
            <span> Всего локаций: <b>{totalCount}</b> </span>
            <span> Сумма : <b>{totalPrice} ₽</b> </span>
          </div>
